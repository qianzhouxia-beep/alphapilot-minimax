...
Output:
  output/icir_prod_v2/icir_ranking.json         ICIR results + weights
  output/icir_prod_v2/scheme_compare.json       3-scheme comparison metrics
  output/icir_prod_v2/scheme_report.html        Interactive HTML report
"""
...
def build_panel(
    ...
    print("  Phase 1: Pre-computing features for all symbols (once each)...", flush=True)
    sym_features = {}
    total = len(by)
    for i, (sym, g) in enumerate(by.items()):
        if i % 100 == 0 or i == total - 1:
            print(f"    [{i+1}/{total}] {sym}", flush=True)
        if len(g) < 100:
            continue
        full = scorer.build_features(g, sym)
        if full is None or len(full) < 30:
            continue
        dates_feat = {}
        for r_idx in range(len(full)):
            fd = str(full.iloc[r_idx].get("date", "")).strip()[:10]
            if fd:
                dates_feat[fd] = r_idx
        sym_features[sym] = {"df": full, "date_idx": dates_feat}
    print(f"  Pre-computed features for {len(sym_features)}/{total} symbols", flush=True)

    for di, d in enumerate(dates):
        reg = regimes.get(d, "unknown")
        n_ok = 0
        for sym, g in by.items():
            if sym not in sym_features:
                continue
            feat_info = sym_features[sym]
            feat_df = feat_info["df"]
            date_idx = feat_info["date_idx"]
            if d not in date_idx:
                continue
            dates_set = set(g["date"].astype(str).str[:10].values)
            if d not in dates_set:
                continue
            ai = int(g.index[g["date"].astype(str).str[:10] == d][0])
            ...
            y2 = sell / buy - 1.0 if buy > 0 else float("nan")
            if str(y2) == "nan":
                continue
