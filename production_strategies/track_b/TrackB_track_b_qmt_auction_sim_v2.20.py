# coding:utf-8
# AlphaPilot -- Track B QMT SIM auction-select strategy v2.20
# =========================================================
# v2.20 (2026-09-16, abr caliber guard parity with Track A v2.49):
#   * MIN_ACTIVE_BUY=0.52 is calibrated on the day-cumulative mootdx/TDX
#     tick caliber. The QMT L1 120-tick fallback is a different caliber ->
...
# v2.19 (2026-09-16, sell weak-regime parity with Track A v2.32/v2.33):
#   * WEAK_REGIME_ENABLE: read market_env.weak_regime from score JSON
#     (fullpool_live / fullpool / candidates). Missing -> False.
#   * Weak day: t2_force floor *= WEAK_FLOOR_MULT (0.6); T+3 cap and
#     extend require price vs MA25 (_weak_cap_sell_ok / no-extend below MA).
#   * Peel unchanged on weak days (same as Track A). SIM-ONLY; live untouched.
#   * Sell-unification P1 (boss order after peel P0).
# v2.18 (2026-09-16, sell peel parity with Track A v2.44/v2.45):
#   * PEEL_PB_MAX=0.02 caps adaptive peel pullback (was min(0.05,...)).
#   * PEEL_NEXT_BAR_CONFIRM=True: first breach only arms; confirm on a later
#     5m bar (toggle False restores first-touch). SIM-ONLY; live tpl unchanged.
#   * Sell-unification P0 (boss): peel cap + next-bar before -4%/TSDOWN/D8.
# v2.17 (2026-09-16, P2 day-high conditional parity with Track A v2.43/v2.47):
#   * DAYHIGH_CONDITIONAL: low_base (ma60_pos<0 & up_low<0.5) -> cap=1.00;
#     else CONF_DAY_HIGH_MAX=0.85. Fields from fullpool_live / survivors.
#   * _p2_decide(..., item=None) so the buy loop can pass the candidate row.
#   * Goal: A/B execution P2 match; selection stays LIM10 vs rank<=3.
# v2.16 (2026-09-16, early P2 vol: same-slot + mild-gap waive / Issue #6):
#   * Plan A: when intraday vol_ma5 burst fails, allow vol_ok if today bar
#     volume / mean(prior 5d SAME tmin slot) >= CONF_VOL_RATIO (1.3).
#     Fixes first-bar structural fail (solo bar vol/ma always = 1.0).
#   * Plan B: 09:36-09:45 AND trigger gap vs prev_close < +4% -> waive the
#     vol burst gate (still requires trend/VWAP/no-chase/day-high/R5).
#     Targets mild early movers (+2~4% or lift from ~-1%) before chase.
#   * _get_m5_bars: allow a single today bar (was n<2 -> None / no_m5),
#     otherwise 09:36-09:39 never reaches _p2_decide.
#   * SIM-ONLY shadow; live tpl unchanged. Logs: [P2] early_vol_waive /
#     same_slot_vol (throttled once/code/day).
# v2.15 (2026-09-14, WB abr soft-gate sink / Issue #6):
#   * Live-pool money_pass: per-bar HARD rejudge (turnover thr(t)=2%*share(t),
#     vr, chg, abr). Server money_flow_pass is soft preference only (sort),
#     not a permanent veto. abr=None / vr invalid -> fail-open.
#   * LIM10 fail-safe: only when HARD local gates all fail THIS bar (not
#     day-flat on frozen 09:36 server flags). Message: hard_all_rejected.
#   * _p2_decide: skip_low_abr (hard) aligned with Track A; thr(t) floor note.
# v2.14 (2026-09-13, Fix C: passorder fill confirmation / ghost-ledger fix):
#   * A passorder ret==0 means "order SUBMITTED", not "filled". v2.13 booked
#     BUY/SELL immediately at the signal price -> ghost ledger (internal
#     position_map / trade log disagree with the broker when the order never
#     fills). Fix C books only after the broker confirms ORDER/DEAL.
#   * Every order now carries a unique userOrderId (-> order_remark) so the
#     order/deal can be linked back. BUY: provisional slot (pending=True), no
#     BUY log yet; _confirm_pending() books the real traded volume/avg price,
#     with _sync_holdings POSITION as the safety net. An order the broker never
#     acknowledges rolls back (drop position, clear lock, allow retry).
#   * SELL / SELL_HALF: the position is NOT popped / reduced until confirmed;
#     an unacknowledged order releases the lock and keeps the position.
#   * _fval() reads BOTH classic QMT m_* fields AND xttrader snake_case fields,
#     because the runtime field naming was still unverified when this landed.
#   * VERIFY_FILL=False restores the legacy "assume filled" behaviour.
#   * Regression: _test_order_confirm.py
...
POS_STATE_PERSIST = (
    ...
    "awaiting_new_high",
    "peel_pending", "peel_touch_bar", "peel_touch_peak",
)
...
# --- Fix C (v2.14, 2026-09-13): passorder fill confirmation ---
# VERIFY_FILL=True  -> book BUY/SELL only after broker ORDER/DEAL confirms
#                      (ghost-ledger fix; see header v2.14 note).
# VERIFY_FILL=False -> legacy behaviour (book immediately at signal price).
VERIFY_FILL = True
...
MIN_TURNOVER = 2.0                # full-day turnover %; floor = MIN_TURNOVER * share(t)
MAX_TURNOVER = 35.0               # turnover ceiling %
MIN_VOL_RATIO = 0.8               # volume ratio floor (invalid/None -> fail-open)
MAX_DROP_PCT = -5.0               # max daily drop %
MIN_MAIN_NET_5D = 0.0             # main 5d net inflow >= 0 (==0 missing -> skip)
USE_TOD_TURNOVER = True           # thr(t)=MIN_TURNOVER*share(t); False -> fixed MIN_TURNOVER

# WB-Mac 2026-09-14 share(t) median table (48 x 5m bars). Intervening bars
# linearly interpolated in bar-index between published anchors.
TURNOVER_SHARE_BY_HHMM = {
    935: 0.0942, 940: 0.1529, 945: 0.2018, 950: 0.2444, 955: 0.2825,
    1000: 0.3144, 1005: 0.3435, 1010: 0.3727, 1015: 0.4018, 1020: 0.4233,
    1025: 0.4447, 1030: 0.4662, 1035: 0.4847, 1040: 0.5033, 1045: 0.5218,
    1050: 0.5364, 1055: 0.5509, 1100: 0.5655, 1105: 0.5775, 1110: 0.5895,
    1115: 0.6015, 1120: 0.6134, 1125: 0.6254, 1130: 0.6374,
    1305: 0.6572, 1310: 0.6700, 1315: 0.6828, 1320: 0.6957, 1325: 0.7085,
    1330: 0.7213, 1335: 0.7336, 1340: 0.7459, 1345: 0.7583, 1350: 0.7706,
    1355: 0.7829, 1400: 0.7952, 1405: 0.8079, 1410: 0.8205, 1415: 0.8332,
    1420: 0.8460, 1425: 0.8587, 1430: 0.8715, 1435: 0.8890, 1440: 0.9065,
    1445: 0.9240, 1450: 0.9493, 1455: 0.9745, 1500: 1.0000,
}
...
# --- v2.17 (2026-09-16, SIM): conditional day-high parity with Track A ---
DAYHIGH_CONDITIONAL = True
CONF_DAY_HIGH_MAX_LOWBASE = 1.00
LOWBASE_MA60_MAX = 0.0
LOWBASE_UP_LOW_MAX = 0.5
CONF_START_MIN = 9 * 60 + 35
CONF_END_MIN = 14 * 60 + 57
CONF_MAX_TURNOVER = 5.0
P2_MODE = True
# --- early P2 vol (v2.16, 2026-09-16): Plan A same-slot + Plan B mild waive ---
# Evidence 002218 2026-09-16: at 09:36 first 5m bar gap=+2.15% day_pos=0.435
# passed every gate except vol (solo-bar ratio always 1.0). Prior same-slot
# and a short mild-gap window recover that entry without lifting the 6%
# no-chase cap for the rest of the day.
EARLY_VOL_SAME_SLOT = True    # Plan A: same-tmin vs prior 5d mean
EARLY_VOL_SAME_SLOT_DAYS = 5
EARLY_VOL_WAIVE = True        # Plan B: waive vol in early mild-gap window
EARLY_VOL_WAIVE_START_MIN = 9 * 60 + 36   # 09:36
EARLY_VOL_WAIVE_END_MIN = 9 * 60 + 45     # 09:45 inclusive
EARLY_VOL_WAIVE_MAX_GAP = 0.04            # trigger c/prev-1 < +4%
...
DEF_TRAIL_ARM = 0.03
DEF_PEEL_PB = 0.015
# v2.18 (2026-09-16, SIM ONLY): peel parity with Track A v2.44/v2.45.
PEEL_PB_MAX = 0.02
...
# --- weak-regime breakdown-sensitive exits (v2.19, parity Track A v2.33) ---
WEAK_REGIME_ENABLE = True
WEAK_FLOOR_MULT = 0.6     # t2_force dynamic floor x0.6 in weak regime
WEAK_TREND_MA = 25        # trend line: 25-day MA
...
def init(C):
    ...
    C._univ_dirty = True
    C.run_count = 0
    # Fix C (v2.14): pending-order registry + unique userOrderId counter
    C.pending_orders = {}
    C._oref_seq = 0
    # Track B gate state
    ...
    print("[INIT] track-B v2.20 (early-vol-A/B+cond-dayhigh+peel-cap2%+nextbar+weak-regime+abr-caliber-guard+LIM10-hard-rejudge+abr+tod-to+path_fade+loud_vol+R5+call-shadow+verify_fill) | acct=" +
...
def handlebar(C):
    ...
    if ts - C._last_resync >= RESYNC_SEC:
        C._last_resync = ts
        _sync_holdings(C)
    # Fix C (v2.14): resolve pending passorder fills/rollbacks before sell/buy
    try:
        _confirm_pending(C)
    except BaseException as e:
...
def _hhmm_from_min(now_min):
    return (int(now_min) // 60) * 100 + (int(now_min) % 60)


def _turnover_share_at_hhmm(hhmm):
    """Lookup share(t); lunch uses 11:30."""
    hhmm = int(hhmm)
    keys = sorted(TURNOVER_SHARE_BY_HHMM.keys())
    if hhmm < keys[0]:
        return float(TURNOVER_SHARE_BY_HHMM[keys[0]])
    if 1130 < hhmm < 1305:
        return float(TURNOVER_SHARE_BY_HHMM[1130])
    if hhmm >= 1500:
        return 1.0
    best = keys[0]
    for k in keys:
        if k <= hhmm:
            ...
            break
    return float(TURNOVER_SHARE_BY_HHMM[best])


def _turnover_floor_pct(now_min):
    """thr(t)=MIN_TURNOVER*share(t) when USE_TOD_TURNOVER else fixed MIN_TURNOVER."""
    if not USE_TOD_TURNOVER:
        return float(MIN_TURNOVER), 1.0
    share = _turnover_share_at_hhmm(_hhmm_from_min(now_min))
    return float(MIN_TURNOVER) * share, share


def _vr_plausible(vr):
    try:
        return vr is not None and float(vr) >= 0.0
...
def _abr_in_range(v):
    """v2.20: abr must be a finite number in [0,1]; else treat as missing."""
    try:
        f = float(v)
    except BaseException:
        return False
...
def _abr_verdict(abr, abr_src):
    """v2.20 abr gate verdict (caliber-aware). Returns:
    'skip'       - calibrated caliber and below MIN_ACTIVE_BUY -> hard reject
    'open_oob'   - out of [0,1] / non-finite -> fail-open (treat as missing)
    'open_uncal' - below floor but non-feed caliber -> fail-open (no veto)
    'pass'       - valid at/above floor, or no data.
    """
    if abr is None:
        return "pass"
    if not _abr_in_range(abr):
        return "open_oob"
    if float(abr) < MIN_ACTIVE_BUY:
...
def _get_m5_bars(C, code):
    ...
    (v2.16) Allow a single today bar so 09:36-09:39 can reach _p2_decide
    (early vol Plan A/B). Previously len<2 returned None -> silent no_m5."""
    today = datetime.now().strftime("%Y%m%d")
...
def _adaptive_params(C, code):
    ...
    # v2.18 (SIM ONLY): cap at PEEL_PB_MAX (2%) instead of 5%.
...
# ====== weak-regime helpers (v2.19, parity Track A v2.32/v2.33) ======
def _weak_regime(C, today):
    """Weak-market flag from score JSON market_env (server state5_q==1).
    Prefer fullpool_live (Track B native), then fullpool, then candidates.
    Cached per day. Missing/unreadable -> False."""
    ...
    if WEAK_REGIME_ENABLE:
        try:
            ...
            names = (
                today + ".fullpool_live.json",
                today + ".fullpool.json",
                today + ".candidates.json",
            )
            fpath = None
            for nm in names:
                ...
                if os.path.exists(p):
                    ...
                    break
            if fpath is None:
                _fetch_remote_fullpool_live(C, today)
                p = os.path.join(sdir, today + ".fullpool_live.json")
                if os.path.exists(p):
                    fpath = p
    ...
    if weak:
        print("[REGIME] weak day: floor x" + str(WEAK_FLOOR_MULT) +
              " extend/cap need price>=MA" + str(WEAK_TREND_MA) +
              " (peel unchanged)")
    return weak


def _ma25(C, code, today):
    """25-day MA of daily closes through yesterday. None on failure."""
    cache = getattr(C, "_ma25_cache", None)
    if cache is None:
        cache = {}
        C._ma25_cache = cache
    key = today + ":" + code
    ...
    try:
        data = C.get_market_data_ex(
            ["close"], [code], period="1d", count=WEAK_TREND_MA + 6,
            subscribe=True)
        if data and isinstance(data, dict) and code in data:
            cl = _col(data[code], "close")
            cl = cl[: len(cl) - 1]
            if len(cl) >= WEAK_TREND_MA:
                ma = sum(cl[-WEAK_TREND_MA:]) / float(WEAK_TREND_MA)
    except Exception:
        ma = None
    cache[key] = ma
    return ma
...
def _weak_cap_sell_ok(C, code, price, today):
    """Weak regime: T+3 hold-cap fires only when price < MA25.
    MA unknown -> legacy sell-at-cap."""
    ma = _ma25(C, code, today)
    if ma and ma > 0:
        return price < ma
    return True
...
def _same_slot_vol_ratio(C, code, tmin):
    """Plan A (v2.16): today volume at 5m slot tmin / mean of prior N days'
    volume at the SAME tmin. Soft-None when history missing.
    Mirrors _r5_call_ratio but keys by tmin instead of day-first bar."""
    if not EARLY_VOL_SAME_SLOT:
        return None
    cache = getattr(C, "_same_slot_vol_cache", None)
    if cache is None:
        C._same_slot_vol_cache = {}
        cache = C._same_slot_vol_cache
    ck = (str(code), int(tmin))
    if ck in cache:
        return cache[ck]
    now = datetime.now()
    ...
    try:
        ...
        for i in range(n):
            ...
            if v > 0:
                day_slot[ds] = v
        cur = day_slot.get(today_str)
        if cur is None or cur <= 0:
            cache[ck] = None
            return None
        hist = [day_slot[ds] for ds in sorted(day_slot.keys())
                if ds < today_str and day_slot.get(ds)]
        hist = hist[-int(EARLY_VOL_SAME_SLOT_DAYS):]
        if not hist:
            cache[ck] = None
            return None
...
def _early_vol_waive_ok(now_min, c, prev):
    """Plan B (v2.16): waive vol burst in 09:36-09:45 when gap < +4%."""
    if not EARLY_VOL_WAIVE:
        return False
    if now_min is None or prev is None or prev <= 0 or c is None or c <= 0:
        return False
    if not (EARLY_VOL_WAIVE_START_MIN <= int(now_min) <= EARLY_VOL_WAIVE_END_MIN):
        return False
    return (float(c) / float(prev) - 1.0) < float(EARLY_VOL_WAIVE_MAX_GAP)
...
def _p2_day_high_max_for(item):
    """v2.17 (SIM): conditional intraday-position cap (parity Track A v2.43).

    Elevated multi-day position -> CONF_DAY_HIGH_MAX (0.85).
    Low-base (below MA60 AND early in rebound) -> relaxed cap.
    Falls back to default when ma60_pos/up_low missing.
    """
...
def _p2_decide(C, code, now_min, item=None):
    ...
    # ABR hard gate at P2 trigger (server abr is soft; QMT owns hard veto).
    # abr=None -> fail-open (do not treat as 0).
    ...
    for i, (tmin, o, c, h, l, v) in enumerate(today_bars):
        ...
        for j in range(max(0, i - 1), i + 1):
            ...
            if bma > 0 and bv > bma * CONF_VOL_RATIO and bret > 0:
                ...
                vol_how = "ma5"
                break
        # Plan B: early mild-gap window waives vol burst (trend/gap/day-high remain).
        if (not vol_ok) and _early_vol_waive_ok(now_min, c, prev):
            vol_ok = True
            vol_how = "early_waive"
        # Plan A: same-slot vs prior days (fixes solo-bar ma5 ratio == 1.0).
        if not vol_ok:
            ssr = _same_slot_vol_ratio(C, code, tmin)
            if ssr is not None and ssr >= CONF_VOL_RATIO:
                vol_ok = True
                vol_how = "same_slot"
        if not vol_ok:
            continue
        if vol_how in ("early_waive", "same_slot"):
            seen = getattr(C, "_early_vol_logged", None)
            if seen is None:
                C._early_vol_logged = set()
                seen = C._early_vol_logged
            lk = str(code) + "|" + vol_how
            if lk not in seen:
                seen.add(lk)
                gap_pct = (float(c) / float(prev) - 1.0) if prev else 0.0
                print("[P2] " + vol_how + " " + str(code)
                      + " t=" + str(tmin)
                      + " gap=" + str(round(gap_pct * 100.0, 2))
                      + " c=" + str(round(float(c), 3)), flush=True)
        if c > prev * (1 + gap_lim):
            continue
            ...
            try:
                ...
                if _key not in C._dayhigh_logged:
                    ...
                    print("[DAYHIGH] " + str(code) + " pos=" + str(round(_dpos, 3))
                          ...
                          + " lim10=" + str((item or {}).get("limit_cnt_10d"))
                          + " veto-bar", flush=True)
            except BaseException:
                pass
            continue
...
def _p2_gate(C, survivors, now_min):
    """P2 money gate. Aligns server money_flow_gate.py (2026-09-14 soft abr).

    Live-pool mode: do NOT freeze on server money_flow_pass. Rejudge HARD
    gates each bar (turnover thr(t), vr, chg, abr). Server pass is soft
    preference for sort only. abr=None / invalid vr -> fail-open.

    Classic-pool mode: same hard gates locally (tod turnover floor).
    """
    out = []
    live_mode = bool(getattr(C, "live_pool_active", False)) and USE_SERVER_GATES
    to_floor, to_share = _turnover_floor_pct(now_min)
    for it in survivors:
        code = it["code"]
        ...
        if live_mode and is_live_row:
            srv = bool(it.get("money_flow_pass"))
            notes.append("srv_pass" if srv else "srv_fail")
        ...
        # HARD rejudge every bar (live + classic)
        if now_min >= GATE_START_MIN:
            ...
            # v2.20 abr caliber guard (see header).
            _v = _abr_verdict(abr, abr_src)
            if _v in ("open_oob", "open_uncal"):
                notes.append("abr=" + ("oob_open" if _v == "open_oob"
                                       else "uncal_open"))
                abr = None
            if abr is not None:
                ...
                notes.append("abr=open")
        to = _get_turnover(C, code)
        if to is not None:
            it["turnover"] = round(to, 2)
            it["turnover_thr_t"] = round(to_floor, 4)
            it["turnover_share_t"] = round(to_share, 4)
            if not (to_floor <= to <= MAX_TURNOVER):
                money_pass = False
                notes.append("to=%s<%.3f" % (round(to, 2), to_floor)
                             if to < to_floor else "to=%s" % round(to, 2))
        ...
        if vr is not None:
            if _vr_plausible(vr):
                ...
                it["volume_ratio"] = None
                notes.append("vr=open")
    ...
    def _key(x):
        ...
        # soft prefer server-pass, then non-retail-chase, then score
        srv_pen = 0
        if live_mode:
            srv_pen = 0 if x.get("money_flow_pass") else 1
        return (not x.get("money_pass"), srv_pen,
                1 if x.get("live_retail_chase") else 0, -float(sc))
    out.sort(key=_key)
    return out
...
def _check_sell(C, now, now_min, today):
    for code, pos in list(C.position_map.items()):
        ...
        # v2.19: weak regime tightens STOP-LOSS / breakdown only; peel unchanged.
        ...
        if now_min >= T2_FORCE_HHMM:
            ...
            if pos.get("t2_extended"):
                # weak regime: cap is breakdown-conditional (price < MA25).
                if (hold_days != 999 and hold_days >= T2_EXTEND_MAX_DAYS
                        and (not weak or _weak_cap_sell_ok(C, code, price, today))):
                    _do_sell(C, code, pos, price,
                ...
                if weak and cap_hit and not _weak_cap_sell_ok(C, code, price, today):
                    cap_hit = False
                ...
                if weak:
                    ma = _ma25(C, code, today)
                    if ma and ma > 0 and price < ma:
                        if pos.get("weak_no_extend") != today:
                            pos["weak_no_extend"] = today
                            print("[EXT] weak-regime " + code + " below ma25=" +
                                  str(round(ma, 2)) + ", no extend")
                    else:
        ...
        if (pos.get("trail_armed") and not pos.get("awaiting_new_high")
            ...
            # v2.18 (SIM ONLY): a strictly new high invalidates a pending touch.
            ...
            if pbk >= pb * 100:
                if PEEL_NEXT_BAR_CONFIRM:
                    # first breach only ARMS; confirm on a LATER 5m bar.
                    ...
                    # first-touch fallback (pre-v2.18).
                    n = pos.get("peel_count", 0)
                    if n >= PEEL_MAX_STEPS or pos["shares"] < 200:
                        _do_sell(C, code, pos, price,
                                 "peel_clear pk=" + str(round(peak, 2)) +
...
# ================= FIX C (v2.14): ORDER FILL CONFIRMATION =================
...
def _next_oref(C, code, today):
    ...
    return ("b" + str(today)[4:8] + dig + str(C._oref_seq))[:20]
    ...
    if kind == "BUY":
        ...
        if not pos.get("pending"):
            ...
            if not rec["logged"]:
                ...
                _log_trade(C, "BUY", code, px, vol, "track_b_auction")
                rec["logged"] = True
...
def _check_buy(C, now, now_min, today, pool):
    ...
    # v2.15: fail-safe only when HARD local gates all fail THIS bar.
    # Soft server flags alone must not day-flat; next bar rejudges.
    if (LIM10_ENABLE and bool(getattr(C, "live_pool_active", False))
            and not money_items):
        # throttle log: once per bar-minute bucket, not once per day
        _flat_key = int(now_min)
        if getattr(C, "_lim10_flat_min", None) != _flat_key:
            C._lim10_flat_min = _flat_key
            print("[LIM10] hard_all_rejected -> flat this bar (no fallback) "
    ...
    for it in picked:
        ...
        fill, reason = _p2_decide(C, code, now_min, it)
        if fill is None:
            if reason in ("no_confirm_eod", "skip_high_turnover",
                          "skip_r5", "skip_low_abr"):
                # permanent abandon: past the eod window, turnover only
                # climbs intraday, R5 gate rejected, or ABR hard fail.
        ...
        try:
            ...
            if VERIFY_FILL:
                ...
                print("[PENDING] BUY " + code + " x" + str(shares) +
                      ...
                      sweet_tag + " oref=" + oref + " -> await fill confirm")
            else:
                print("[BUY] " + code + " x" + str(shares) + " @ " +
