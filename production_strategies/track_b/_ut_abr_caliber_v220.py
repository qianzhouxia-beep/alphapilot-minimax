"""Unit tests: Track B v2.20 abr caliber guard (2026-09-16).

Run: /Users/AlphaPilot/.venv/bin/python _ut_abr_caliber_v220.py
"""
import importlib.util
import math
import os

HERE = os.path.dirname(os.path.abspath(__file__))
MOD = os.path.join(HERE, "TrackB_track_b_qmt_auction_sim_v2.20.py")

spec = importlib.util.spec_from_file_location("b220", MOD)
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)

fails = []


def chk(name, got, want):
    ok = got == want
    print(("  [PASS] " if ok else "  [FAIL] ") + name + " got=" + repr(got)
          + " want=" + repr(want))
    if not ok:
        fails.append(name)


print("== _abr_in_range ==")
chk("0.5 in range", m._abr_in_range(0.5), True)
chk("1.5 out", m._abr_in_range(1.5), False)
chk("neg out", m._abr_in_range(-0.1), False)
chk("nan out", m._abr_in_range(float("nan")), False)

print("== _abr_verdict (ABR_CALIBRATED_SRC_ONLY=True) ==")
m.ABR_CALIBRATED_SRC_ONLY = True
chk("None -> pass", m._abr_verdict(None, "none"), "pass")
chk("feed low -> skip", m._abr_verdict(0.40, "feed"), "skip")
chk("feed ok -> pass", m._abr_verdict(0.60, "feed"), "pass")
chk("l1 low -> open_uncal", m._abr_verdict(0.40, "l1"), "open_uncal")
chk("l1 ok -> pass", m._abr_verdict(0.60, "l1"), "pass")
chk("oob -> open_oob", m._abr_verdict(2.5, "feed"), "open_oob")

print("== _abr_verdict (ABR_CALIBRATED_SRC_ONLY=False) ==")
m.ABR_CALIBRATED_SRC_ONLY = False
chk("l1 low -> skip", m._abr_verdict(0.40, "l1"), "skip")

print("===== failed=%d =====" % len(fails))
raise SystemExit(1 if fails else 0)
