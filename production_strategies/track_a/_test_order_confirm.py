#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix C regression test: Track A passorder fill confirmation (v2.46).

Ghost ledger (2026-09-12/13, issue #6): passorder ret==0 only means "order
submitted". v2.45 booked BUY/SELL immediately at the signal price, so a
rejected/canceled order still created a position + trade-log entry that the
broker never had. v2.46 confirms via ORDER/DEAL before booking. Mirrors the
Track B v2.14 fix so the A/B sim comparison stays on equal footing.

Scenarios (A/B RED before v2.46, GREEN after; C/D/E regression):
...
SIM = os.environ.get("TA_SIM_FILE") or os.path.join(
    HERE, "TrackA_track_a_qmt_full_chain_sim_v2.46.py")

spec = importlib.util.spec_from_file_location("ta_sim_fixc", SIM)
m = importlib.util.module_from_spec(spec)
sys.modules["ta_sim_fixc"] = m
...
TMP = tempfile.mkdtemp(prefix="ta_fixc_")
...
m.COOLDOWN_FILE = os.path.join(TMP, "cooldown.json")
m.FUSION_CLOSED_LOG = os.path.join(TMP, "fusion.jsonl")
m.LEDGER_DAILY = os.path.join(TMP, "ledger.json")
m.VERIFY_FILL = True
