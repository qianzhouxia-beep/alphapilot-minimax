# coding:utf-8
# AlphaPilot -- Track A QMT sim full-chain strategy v2.49 (gene + path_fade + loud_vol + R5 + D1-D5 stop-bleeding + D8 observe + B3/B1 shadow + TSDOWN/D8-3C + conditional day-high + peel-cap + peel-nextbar + verify_fill + early-vol-A/B; FIXED_STOP off; abr caliber guard)
# v2.49 (2026-09-16, SIM ONLY): abr caliber guard -- MIN_ACTIVE_BUY=0.52 is
#   calibrated on the day-cumulative mootdx/TDX tick caliber. The QMT L1
#   fallback (last 120 ticks) is a DIFFERENT caliber; applying 0.52 to it
#   mass-abandoned candidates (000722 2026-09-16; feed missing 09-11..).
...
# v2.48 (2026-09-16, SIM ONLY): FIXED_STOP_ENABLE=False -- disable D3-1 -4%
#   intraday fixed stop (boss: too blunt on first sellable day / T+1; do not
#   deploy; more validation required before any re-enable; do NOT port to B).
#   D8 observe exemption / observe_floor keep code paths but are inert while
#   fixed stop is off (disaster floor only mattered as -4% bypass).
# v2.47 (2026-09-16, SIM ONLY): P2 early-vol Plan A/B parity with Track B
#   v2.16 -- same-slot vol + 09:36-09:45 mild-gap (<+4%) vol waive + allow
#   single today 5m bar in _get_m5_bars. Selection (rank<=3) unchanged;
#   execution P2 aligned with B for unified live rollout later.
...
#   path_fade = recent limit + T-1 gap-up HOLC (server stamped). No exemption.
# v2.34 (2026-09-03): MAX_CAND_RANK=3. Server gene-reranks Top10 by
# limit_cnt_10d + ma25_slope + ret_10d (T-1); fund is gate only. Aligns
# with live v2.34-tpl. 34-day authentic: gene<=3 sweet spot vs <=2/<=5.
# v2.33 (2026-09-03): profit-taking (peel) NOT tightened in weak regime --
# user directive: the weak-regime side is stop-loss / breakdown only; the
# profit-taking side stays exactly as v2.30. WEAK_PB_MULT removed.
# v2.32 (2026-09-03): trend line = 25-day MA, not day VWAP (aligned with
# Track A QMT v2.32-tpl). Day VWAP is intraday noise -- too strict as the
# breakdown test; trend breaks only when price loses the 25-day MA (daily
# closes through yesterday). Hold-cap drops the ret<=0 clause -- trend is
# the only criterion. v2.31's floor x0.6 / peel x0.7 tightening unchanged.
# v2.31 (2026-09-03): weak-regime breakdown-sensitive exits (aligned with
...
# v2.29 changes vs v2.28 (2026-09-01, rotation off):
...
# v2.28 changes vs v2.27 (2026-09-01, rank<=2 P2 gate):
...
# --- early P2 vol (v2.47, 2026-09-16): Plan A same-slot + Plan B mild waive ---
# Parity with Track B v2.16. Evidence 002218 2026-09-16 + boss ask to unify
# A/B execution P2 (selection stays different: A rank<=3 vs B LIM10).
...
# --- weak-regime breakdown-sensitive exits (v2.33, 2026-09-03) ---
...
ROTATION_ENABLE = False         # v2.29: off (rank<=2 ~1 buy/day; no kick-out)
...
# D3-1 intraday fixed stop (T+1+ = first sellable day after buy), sell when
# ret <= -FIXED_STOP_PCT. v2.48: OFF by default (boss 2026-09-16).
FIXED_STOP_ENABLE = False
FIXED_STOP_PCT = 4.0
...
def init(C):
    ...
    print("[INIT] track-A qmt-sim v2.49 (gene+R5+D1-D5+D8, rank<=3, cond-dayhigh, early-vol-A/B, peel-cap2%+nextbar+verify_fill, fixed-stop-OFF, abr-caliber-guard) | acct=" + ACCOUNT_ID +
...
def _abr_in_range(v):
    """v2.49: abr must be a finite number in [0,1]; else treat as missing."""
    try:
        f = float(v)
    except BaseException:
        return False
...
def _abr_verdict(abr, abr_src):
    """v2.49 abr gate verdict (caliber-aware). Returns:
...
def _get_m5_bars(C, code):
    ...
    (v2.47) Allow a single today bar so 09:36-09:39 can reach _p2_decide
    (early vol Plan A/B; parity with Track B v2.16)."""
    today = datetime.now().strftime("%Y%m%d")
...
# ====== weak-regime helpers (v2.32, 2026-09-03: MA25 trend line) ======
...
def _same_slot_vol_ratio(C, code, tmin):
    """Plan A (v2.47): today volume at 5m slot tmin / mean of prior N days'
    volume at the SAME tmin. Soft-None when history missing.
    Parity with Track B v2.16."""
...
def _early_vol_waive_ok(now_min, c, prev):
    """Plan B (v2.47): waive vol burst in 09:36-09:45 when gap < +4%."""
    ...
    for i, (tmin, o, c, h, l, v) in enumerate(today_bars):
        ...
        for j in range(max(0, i - 1), i + 1):
            ...
            if bma > 0 and bv > bma * CONF_VOL_RATIO and bret > 0:
                ...
                vol_how = "ma5"
                break
        # Plan B: early mild-gap window waives vol burst (parity Track B v2.16).
...
def _check_sell(C, now, now_min, today):
    for code, pos in list(C.position_map.items()):
        ...
        # v2.48: gated by FIXED_STOP_ENABLE (default False; boss off-deploy).
        if FIXED_STOP_ENABLE and ret <= -FIXED_STOP_PCT:
...
def _check_buy(C, now, now_min, today, cands):
    # v2.28: only rank 1-2 may enter P2 / rotation. Filter BEFORE sweet-zone
