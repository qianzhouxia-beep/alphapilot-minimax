# -*- coding: utf-8 -*-
"""v2.16 rotation hardening tests (2026-08-18, DSH review of the sell-side rework):
  1. T+1 protection: a holding bought yesterday (hold_days=1) is IMMUNE to rotation
     even if it is the worst performer (ROTATION_MIN_HOLD_DAYS 2).
  2. Hysteresis gate (ROTATION_WEAK_GATE): a "relatively weakest but healthy" holding
     (ret>0, day>0, above day-VWAP, no early-exit flag) is NOT rotated out.
...
# Production default is ROTATION_ENABLE=False (v2.29). Tests cover helpers.
mod.ROTATION_ENABLE = True

# ---- in-memory order locks so the daily-cap path is exercised ----
LOCKS = {}
...
PREV_CLOSE = {}      # code -> prev close
VWAPS = {}           # code -> day VWAP (None = unavailable)
SELL_CALLS = []      # (code, reason)
...
C = _C()
C.position_map = {
    "600100.SH": mkpos("20260817", 100.0),   # hold 1 day - CLEARLY weakest, must be immune
...
QUOTES["600100.SH"] = (80.0, 100.0, 82.0, 90.0)   # -20%, crashing
QUOTES["600002.SH"] = (96.0, 100.0, 96.0, 97.0)   # -4%
...
C2 = _C()
C2.position_map = {
    "600001.SH": mkpos("20260816", 100.0),   # +1% day, above vwap
    "600002.SH": mkpos("20260816", 100.0),   # +0.5% day (relatively weakest but healthy)
    "600003.SH": mkpos("20260816", 100.0),
    "600004.SH": mkpos("20260816", 100.0),
}
# all profitable, all above day-VWAP, no early flag, no vwap break
...
# weak pool: two genuinely weak names (both -4%), rotation wants 1 -> cap blocks the 2nd
QUOTES.update({
...
# restore the sold slot, then try again on the same day
