...
#   Same as sim v2.35 / Track B path_fade. No lim10 exemption (server).
# v2.34-tpl (2026-09-03): MAX_CAND_RANK=3. Server candidates.json is
# gene-reranked (limit_cnt_10d + ma25_slope + ret_10d, T-1) inside Top10;
# fund stays a pass/fail gate, not a sort key. P2 may fire on new ranks 1-3.
# Evidence: 34 authentic days -- gene<=3 fills 2 seats 65%, WORST T+1 still
# +1.8%; gene<=2 too narrow; gene<=5 WORST T+1 turns negative.
# v2.33-tpl (2026-09-03): profit-taking (peel) is NOT tightened in weak
# regime -- user directive: the weak-regime side is stop-loss / breakdown
# only; the profit-taking side stays exactly as v2.30. WEAK_PB_MULT removed.
# v2.32-tpl (2026-09-03): trend line = 25-day MA, not day VWAP. User
# directive: day VWAP is an intraday line -- too strict as the breakdown
# test; the short-term trend breaks only when price loses the 25-day MA
# (daily closes through yesterday). All weak-regime "trend-intact" checks
# now use MA25; the hold-cap drops the ret<=0 clause -- trend is the only
# criterion. v2.31's floor x0.6 / peel x0.7 tightening is unchanged.
# v2.31-tpl (2026-09-03): weak-regime breakdown-sensitive exits. User
# directive: NO hard time exits for trend-intact positions -- regime only
# changes how SENSITIVE the breakdown exits are, never a calendar deadline.
#   * weak_regime flag from {date}.candidates.json market_env (server stamps
#     yesterday's all-A 5d main-flow quintile state5_q; weak = q==1). Missing
#     or unreadable -> non-weak (legacy behavior, safe).
#   * weak regime: t2_force dynamic floor x0.6 (cut losers faster); peel
#     pullback x0.7 (lock winners faster); T+2 14:45 extension granted only
#     to trend-intact names (price >= day VWAP) -- a broken name is not
#     force-sold here, vwap_weak_early fires next morning as before; the T+3
#     hold-cap becomes a breakdown check (intact keeps running, daily recheck).
#   * non-weak regime: everything unchanged.
#   * Data basis: bt_weak_sell_tight 2026-09-03 (prod Top2, 41 picks): weak
#     days best exit T+1 close (+3.96%), decay T+2~T+3 (median -1.73%);
#     non-weak holding wins (T+5 +13.84%). Weak-regime edge is in faster
#     breakdown exits, not in time caps.
# v2.30-tpl (2026-09-02): vwap_weak_early needs TWO still-below minutes.
# First 09:35-09:50 tick below vwap_ref only arms (log "wait 2nd"); a later
# minute still below sells. Same-minute re-polls do not count. Recover
# (>=vwap_ref) still cancels. 300475 09-02 sold the 09:36 low on first tick.
# v2.29-tpl (2026-09-01): ROTATION_ENABLE=False. rank<=2 buys ~1/day so the
# weakest-name kick-out (make room at MAX_HOLDINGS) is no longer needed and
# caused white-sells (002058 08-31, TDX 002015 +8.3% after rotation).
# v2.28-tpl (2026-09-01): MAX_CAND_RANK=2 -- only 09:35 candidates.json rank
# 1-2 may enter P2 (incl. rotation worth_buy). Rank 3+ never race. Missing
# rank is skipped. P2 itself is unchanged. Evidence: 22 settled days
# 2026-07-27~08-31, rank<=2 T+1 +1.62% t=2.14 vs rank<=10 +0.67%.
# v2.27-tpl (2026-08-31): _hold_days counts TRADING days, not calendar days.
...
# v2.26-tpl (2026-08-31): vwap_weak_early next-morning confirm -- same as sim
# v2.25. Sell at next open only if the live price is still BELOW the day-VWAP
...
# v2.25-tpl (2026-08-29): P2 sweet-zone trigger priority -- same as sim v2.25
# (SWEET_ZONE_MODE / SWEET_GAP_LO / SWEET_GAP_HI; _order_cands_by_sweet reorders
# candidates before the daily buy-slot race; BUY logs tagged [SWEET]). Trigger
# order preference only, no scoring change. Live templates historically lagged
# sim on P2 features; this aligns live with sim for the sweet-zone layer.
# v2.24-tpl (2026-08-28): sell jsonl falls back to {buy_date}.candidates.json
...
# v2.20 (2026-08-26): P2 board-aware gap + day-high guard (same as sim v2.20).
# v2.19 (2026-08-20, T+1 winner force-sell fix): same as sim -- _sync_holdings
...
# (ACCOUNT_ID, ACCOUNT_NAME, board access, position size) -- everything else
...
#   candidate for today).
# v2.12 changes vs v2.11 (2026-08-15, synced from sim v2.12):
#   FIX over-sized sell orders. _sync_holdings now reads the broker tradable
#   qty (m_nCanUseVolume) into pos["can_use"]; _do_sell/_do_sell_half cap the
#   order to can_use. Real case 2026-08-14: stale in-memory shares fired a
#   80900-share sell when only 40500 were sellable -> broker rejected with
#   [251005] insufficient available qty. Same-day buys also failed the T+1 skip because QMT
#   leaves m_strOpenDate empty: buy_date is now inferred from can_use<vol.
# v2.11 changes vs v2.10 (2026-08-14):
#   FIX critical "silent no-buy" bug (synced from sim v2.11): passorder
#   REJECTED orders still wrote BUY lock + position + trade log + today_bought,
#   because `if ret != 0` only printed. A rejected order now prints
#   "[BUY] ... REJECTED ret=.. (no lock written)" and never consumes a buy slot.
#   Sell side: rejected passorder(24) no longer writes lock / fake SELL trade /
#   drops position. Added visible logs for silent exits and an INIT lock summary.
...
# =========================================================
# >>> CONFIG  (edit this block per account, then deploy) <<<
# =========================================================
...
FUSION_SOURCE = "qmt_" + ACCOUNT_TAG

# --- position sizing (v2.1: sized on TOTAL ASSETS via m_dBalance) ---
MAX_HOLDINGS = 4
MAX_DAILY_BUY = 2
MAX_CAND_RANK = 3              # gene-reranked candidates.json rank 1-3 may enter P2; 0=off
POSITION_PCT = 0.20            # each buy = 15% of total assets
...
# --- P2 sweet-zone priority (v2.25-tpl, 2026-08-29, same as sim v2.25) ---
...
# --- weak-regime breakdown-sensitive exits (v2.33-tpl, 2026-09-03) ---
WEAK_REGIME_ENABLE = True
WEAK_FLOOR_MULT = 0.6     # t2_force dynamic floor x0.6 in weak regime
WEAK_TREND_MA = 25        # trend line: 25-day MA (user's line, 2026-09-03)
# v2.33: peel (profit-taking) deliberately NOT tightened in weak regime.

# --- T+2 conditional force-close + dynamic weakness rotation (v2.16-tpl, 2026-08-18) ---
...
ROTATION_ENABLE = False         # v2.29-tpl: off (rank<=2 ~1 buy/day; no kick-out)
...
def _get_m5_bars(C, code):
    ...
    kept. (v2.14-tpl, 2026-08-18) QMT get_market_data_ex(count=48) without
...
def _ma25(C, code, today):
    """v2.32: 25-day MA of daily closes through yesterday (today's partial
    bar dropped, same convention as _wyckoff_distribution). Cached per
    (day, code) on C. None on data failure."""
    ...
    try:
        ...
        if data and isinstance(data, dict) and code in data:
            ...
            cl = cl[: len(cl) - 1]  # drop today's partial bar
...
def _weak_cap_sell_ok(C, code, price, today):
    """v2.32 weak regime: the T+3 hold-cap fires only on a confirmed trend
    break -- price below the 25-day MA (user's trend line, 2026-09-03).
    Trend-intact positions keep running and are rechecked each day -- no
    calendar deadline, no underwater clause. MA25 unknown -> legacy
    behavior (sell at cap)."""
    ma = _ma25(C, code, today)
    if ma and ma > 0:
        return price < ma
    return True  # MA25 unknown -> keep legacy behavior (sell at cap)
...
def _filter_cands_by_max_rank(cands):
    """Keep rank 1..MAX_CAND_RANK; drop path_fade and loud_vol (v2.36-tpl)."""
    if MAX_CAND_RANK <= 0:
        return list(cands or [])
    kept = []
    for it in (cands or []):
        r = int(it.get("rank") or 0)
        if not (0 < r <= MAX_CAND_RANK):
            continue
        if it.get("path_fade"):
            print("[PATH] skip " + str(it.get("symbol")) +
                  " path_fade rank=" + str(r), flush=True)
            continue
        if it.get("loud_vol"):
            print("[LOUD] skip " + str(it.get("symbol")) +
                  " loud_vol rank=" + str(r) +
                  " vmr=" + str(it.get("vol_ma_ratio")), flush=True)
            continue
        kept.append(it)
    return kept
...
def _order_cands_by_sweet(C, cands):
    """Trigger-order preference (v2.25-tpl): sweet-zone candidates first when they
...
def _p2_decide(C, code, now_min):
    ...
    # Rolling session low replaces P935 as the trend reference (v2.21-tpl).
    amt_cum = 0.0
    vol_cum = 0.0
...
def _check_sell(C, now, now_min, today):
    for code, pos in list(C.position_map.items()):
        ...
        if not pos.get("vwap_broken") and now_min >= VWAP_CONFIRM_MIN:
            ...
            if vw and vw > 0 and price < vw:
                ...
                print("[VWAP] " + code + " day-vwap broken px=" +
                      ...
                      " ret=" + str(round(ret, 1)) + "%")
        # v2.26-tpl: next-morning confirm. Unconditional next-open sell sold
        # the day's low on all 3 real triggers (QMT sim 08-31). Sell only if
        # the live price is still below the recorded reference; recover -> cancel.
        ...
        # v2.33: weak regime tightens the STOP-LOSS / breakdown side only
        # (floor + MA25 checks). Profit-taking (peel) is unchanged.
        ...
        # T+2 conditional force-close (v2.17-tpl, 2026-08-19): dynamic floor force-sell + profit extend T+3 (peel guards)
        if now_min >= T2_FORCE_HHMM:
            ...
            if pos.get("t2_extended"):
                ...
                # v2.31: weak regime turns the cap into a breakdown check --
                ...
                if weak:
                    # v2.31: weak regime raises the floor (less negative) so
                ...
                # v2.31: weak regime makes the hold-cap breakdown-conditional.
                ...
                if cap_hit or ret <= hs * 100:
                    _do_sell(C, code, pos, price,
                             "t2_force_after_extend " + str(round(ret, 1)) + "%")
                    continue
                # v2.32: weak regime grants the extra day only to trend-intact
                # names (price >= 25-day MA). A broken name is NOT force-sold
...
def _check_buy(C, now, now_min, today, cands):
    # v2.28-tpl: only rank 1-2 may enter P2 / rotation. Filter BEFORE sweet-zone
    # reorder so rank 3+ cannot take a daily slot or trigger rotation.
    n_all = len(cands or [])
    cands = _filter_cands_by_max_rank(cands)
    if n_all and C.run_count % 60 == 1:
        print("[RANK] max=" + str(MAX_CAND_RANK) + " keep=" + str(len(cands)) + "/" + str(n_all))
    # v2.25-tpl: sweet-zone trigger priority -- reorder candidates before the
    ...
    for item in cands:
        ...
        try:
            ...
            if ret != 0:
                ...
                # (2026-08-14 sim case: account had ZERO broker orders but
                # lock+trades were written at 11:07, then no buys all day).
    ...
    try:
        ...
        if pos and isinstance(pos, dict) and pos.get("fusion_scores"):
            ...
            try:
                ...
                pass
        # dedup: same signature within window is a re-fired order, skip
...
#
# Scores are shared from the server: SCORE_DIR (local copy) or
