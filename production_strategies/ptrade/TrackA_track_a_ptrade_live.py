# -*- coding: utf-8 -*-
# AlphaPilot -- Track A Ptrade LIVE strategy TEMPLATE v1.7-tpl (vwap 2nd confirm)
# File: TrackA_track_a_ptrade_live.py
# =========================================================
# v1.7-tpl (2026-09-02): vwap_weak_early needs TWO still-below minutes.
# Aligned with QMT v2.30-tpl.
# v1.6-tpl (2026-09-01): ROTATION_ENABLE=False -- aligned with QMT v2.29-tpl.
# v1.5-tpl (2026-09-01): MAX_CAND_RANK=2 -- only 09:35 candidates.json rank
# 1-2 may enter P2 (incl. rotation worth_buy). Rank 3+ never race. Missing
# rank is skipped. P2 itself is unchanged. Aligned with QMT v2.28-tpl.
# v1.4-tpl (2026-08-31): trading-day hold fix, aligned with QMT/TDX.
...
# v1.3-tpl (2026-08-31): vwap_weak_early next-morning confirm -- aligned with
# QMT/TDX. Sell at next open only if the live price is still BELOW the day-VWAP
...
# v1.2-tpl (2026-08-27): persist sell-side metadata (sim v1.2).
# v1.1 (2026-08-20, T+1 winner force-sell fix): same as sim -- _sync_holdings
# recovers buy_date from the trade log when open date is empty after a restart;
# _check_sell T+2 maturity check guards hold_days != 999 (000651 08-20 case).
# TEMPLATE: deploy one copy per live Ptrade account. Edit ONLY the CONFIG
...
#
# Deploy: paste into a Ptrade strategy (LIVE trading module), set period to
# "minute", then run. Validate in SIM first (TrackA_track_a_ptrade_sim.py),
# then clone to LIVE and edit only the CONFIG block.
# Pure ASCII. No f-strings (legacy Ptrade VM = Py3.5+).
#
# =========================================================
# >>> CONFIG  (edit this block per account, then deploy) <<<
# =========================================================
# Short ASCII tag used ONLY for local file names (order locks / trade log)
# in the research path, so two strategies on the same broker account never
# share files. e.g. "live_alice" / "sim_my".
ACCOUNT_TAG = "live"

# Board permission for THIS account (v2.4 port). False = skip that board's
# candidates (no wasted buy slot, no rejected order). Ptrade may reject
# orders for boards the account cannot trade.
# STAR = 688/689, ChiNext = 300/301, BSE = 8xx/4xx/920.
ALLOW_STAR = False            # (live default: no STAR access unless granted)
ALLOW_CHINEXT = False         # (live default: no ChiNext access unless granted)
ALLOW_BSE = False             # (live default: no BSE access unless granted)
...
# --- account / sizing (per-account risk) ---
MAX_HOLDINGS = 4
MAX_DAILY_BUY = 2
...
# --- board permission filter (values set in the CONFIG block above) ---
# (kept here as aliases so the logic below reads them; edit CONFIG only)
...
# --- rotation (v2.16; off since v1.6-tpl, rank<=2 ~1 buy/day) ---
...
def _lock_path():
    rp = _research_path()
    if rp:
        return rp + "/ptrade_tracka_" + ACCOUNT_TAG + "_order_locks.json"
    return ""
...
def _trade_log_path():
    rp = _research_path()
    if rp:
        return rp + "/ptrade_tracka_" + ACCOUNT_TAG + "_trades.json"
    return ""
...
def _pos_state_path():
    rp = _research_path()
    if rp:
        return rp + "/ptrade_tracka_" + ACCOUNT_TAG + "_pos_state.json"
    return ""
...
def _filter_st_out(out):
    ...
    for it in out:
        ...
        keep.append(it)
    if len(keep) != len(out):
        log("[CAND] ST/退市硬过滤剔除 " + str(len(out) - len(keep)))
    return keep
...
def _hold_days(pos, today):
    ...
    or 'YYYY-MM-DD'. 999 on missing/unparseable. v1.4-tpl: weekends + 2026
    A-share closures excluded (see _trading_days_between), so Friday buy = T+1 on Monday."""
...
def initialize(context):
    ...
    try:
        ...
        pass
    _log("[INIT] track-A ptrade v1.7-tpl (P2 + rank<=2, vwap 2nd) LIVE | tag=" +
         ACCOUNT_TAG + " | holdings=" + str(len(context.position_map)) +
...
# =========================================================
# 1) Copy this file for each live Ptrade account you manage.
# 2) In the CONFIG block at the top, set:
#      ACCOUNT_TAG  -> short ASCII tag, e.g. "alice_live"; used ONLY in the
#                      research-path lock / trade-log file names so accounts
#                      never share files
#      ALLOW_STAR / ALLOW_CHINEXT / ALLOW_BSE -> board access for that account
#      POSITION_PCT / MAX_HOLDINGS / MAX_DAILY_BUY -> per-account risk
# 3) In Ptrade, create a strategy with period="minute", paste this file,
#    attach a LIVE trading account, and start it.
# 4) Data delivery: configure EITHER
#      a. HTTP whitelist -- ask the broker to allow outbound HTTPS/HTTP to
#         your AlphaPilot server nginx (REMOTE_SCORE_BASE), OR
#      b. upload_files -- schedule your local machine to sync the server's
#         score JSONs into the Ptrade client "scheduled upload" directory;
#         the strategy reads them via get_research_path()/upload_files.
#    The strategy tries HTTP first, then upload_files, then research root.
# 5) Check the log prints "[INIT] track-A ptrade v1.7-tpl ... LIVE | tag=...".
#
# Per-account research-path files (auto-created when the VM allows writes):
#   <research>/ptrade_tracka_<TAG>_order_locks.json  - daily order locks
#   <research>/ptrade_tracka_<TAG>_trades.json       - trade journal
# If the sandbox denies writes, locks/log stay in-memory for the session
# (order dedup then relies on broker order ids + position sync).
#
# Scores are shared from the server -- no per-account setup needed.
