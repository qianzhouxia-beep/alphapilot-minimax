...
| **ABR 门（v2.13 新增；v2.49/v2.20 口径护栏 + 证据日志）** | P2 触发后连续竞价时段（≥09:30）检查主动买占比；**仅当口径=标定源（`src=feed`，即 `l2_feed` 文件通道）时** `<0.52 → 当日放弃`（`skip_low_abr`）。回退 QMT L1 末 120 笔近似是**另一口径**，低于 0.52 **fail-open**（不否决）；abr ∉ [0,1]/非有限同样 fail-open。**残留（待重标定）**：0.52 标定于 P2_cum **全日累计窗**，而今生产者 `tencent_tick_feed.py` 的 abr = **末 800 笔滚动窗** → P2 日志/item 记 `src=feed win=N`（E20/E23）；本轮**刻意不重标定**（无同日 TDX↔腾讯对照，不拍数）。TDX 用盘口买一档量占比近似。**软门**：ABR 不可用不拦截 |
| 执行 | 09:35~14:57 观察窗，**仅 Top2 内**先到先得，日限 2 笔、持仓 4 只、每笔 15% 总资产；两只都不过 P2 则不买 |
| 数据 | `C:\alphapilot\l2_feed\{date}.json`（独立 feed 进程；2026-09-15 起由 **`tencent_tick_feed.py`** 替代 `mootdx_feed.py`，路径/字段不变，abr 窗口 = 末 800 笔） |
