...
- [`inbox/2026-09-16-overnight-signal-forward-roll-fix.md`](./inbox/2026-09-16-overnight-signal-forward-roll-fix.md) — **隔夜美股信号前滚缺陷修复（ingest v1.1）**：cn 09-16 被塞 us 09-14 陈旧值（上游新浪 05:30 尚未发布 09-15、当晚才可见 × `prev_us_session` 回扫静默复用）→ **预期美市日缺失即显式 `g1_flag=stale`、绝不前滚**；回填 us 09-15（NVDA +0.5736% / QQQ −0.6543%，flag `ai_rebound_watch`→`none`，`backfilled=true`）；05:30 外增 07:00/09:20 兜底重跑。规格 `ops/gate_spec_G1G2_20260915.md` §7
- [`inbox/2026-09-17-lhb-empty-poisoning-fix.md`](./inbox/2026-09-17-lhb-empty-poisoning-fix.md) — **lhb_stale 复发修复（pull_lhb v1.2）**：未发布时的空响应被永久记入 `fetched_ok` → 之后所有 run（含预检 repair）跳过，缺口累积到 `lag≥2`；且 00:30 会把「今天」预标致隔日复发。改为**以 K 线日历判定**（交易日/今天空响应不确认、留待重试；周末假日才确认）+ **自愈**剔除毒化日期 + **23:30 兜底 cron**。回填 09-15/16，max=`09-16`=kline，预检全绿
- [`inbox/2026-09-17-us-ingest-firstrun-and-anomaly-verdicts.md`](./inbox/2026-09-17-us-ingest-firstrun-and-anomaly-verdicts.md) — **v1.1 首跑验证 + 两异常裁定**：信号出 `stale`（不复制旧值）✓，根因是**新浪分标的发布滞后**（09:20 QQQ 有 us 09-16、NVDA 没有）→ 回填 `cn=09-17←us=09-16`（+0.8154%/+0.0255%, `none`）+ 晨间重试加密至 7 跑；`event_calendar` **非空**（list schema，G2 `is_gap_day=true` 正常，WB 误报）；`sector_heat base_day` 是**基期语义**非 stale
- [`inbox/2026-09-17-sector-rotation-reversal-study.md`](./inbox/2026-09-17-sector-rotation-reversal-study.md) — **板块轮动「流出后反弹」三轮检验**：**资金流零增量**（与当日涨跌相关 +0.49、跌板块 87~90% 是流出、正交化 IC≈0）；**反转是阶段性**（当前 08-07~09-17 显著 T1 Δ+0.41% t=+2.25，但 415 日全历史 Δ+0.010% t=0.18、长期偏延续、科技为负）；**候选池跑输与板块无关**（池内分层均不显著；9月 ex_T1 −1.375% t=−2.39，8月不显著）。报告 `bt_research/_sector_rotation/SECTOR_ROTATION_STUDY_20260917.md`
