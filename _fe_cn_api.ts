...
export async function fetchLiveRecommend(top_n: number = 50): Promise<LiveRecommendResponse> {
  return apiFetch<LiveRecommendResponse>(`${CN_ENDPOINTS.recommendLive}?top_n=${top_n}`);
}
...
export type LiveOrderTicket = {
  ...
  live_main_net?: number;
  money_phase_label?: string;
...
export type LiveOrdersResponse = {
  tickets: LiveOrderTicket[];
  pending: LiveOrderTicket[];
  pending_n: number;
  expired?: LiveOrderTicket[];
  expired_n?: number;
  expire_hhmm?: string;
  broker: any;
...
function authHeaders(): Record<string, string> {
  ...
  try {
    ...
    if (token && !String(token).includes("mock_signature")) {
      return { Authorization: `Bearer ${token}` };
    }
    // 兼容真 JWT 存在其它 key
    const t2 = localStorage.getItem("alphapilot_token") || localStorage.getItem("ap_token");
    if (t2) return { Authorization: `Bearer ${t2}` };
  } catch {}
  return {};
}

async function apiFetchAuth<T>(url: string, init?: RequestInit): Promise<T> {
  const res = await fetch(url, {
    cache: "no-store",
    ...init,
    headers: {
...
export async function fetchLiveOrders(params?: {
  status?: string;
  today_only?: boolean;
}): Promise<LiveOrdersResponse> {
  const q = new URLSearchParams();
  if (params?.status) q.set("status", params.status);
  if (params?.today_only === false) q.set("today_only", "false");
  const qs = q.toString();
  return apiFetchAuth<LiveOrdersResponse>(
    `/api/v1/cn/live-orders${qs ? `?${qs}` : ""}`
  );
}

export async function approveLiveOrders(payload: {
  ticket_ids: string[];
  weights?: Record<string, number>;
  execute_now?: boolean;
}): Promise<any> {
  return apiFetchAuth<any>("/api/v1/cn/live-orders/approve", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export async function rejectLiveOrders(payload: {
  ticket_ids: string[];
  reason?: string;
}): Promise<any> {
  return apiFetchAuth<any>("/api/v1/cn/live-orders/reject", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export async function fetchBrokerConnection(): Promise<any> {
  return apiFetchAuth<any>("/api/v1/cn/broker-connection");
}

export async function saveBrokerConnection(payload: {
  adapter?: string;
  enabled?: boolean;
