...
async def get_recommend():
    ...
    if from_morning:
        # 已是当日 morning_live Top2：不再二次资金硬滤，避免网页与交易名单不一致
...
async def get_paper_trading(user: dict = Depends(get_current_user)):
    ...
    for t in trades:
        ...
        if t.get("time","").startswith(today) and "卖出" in a:
            daily_settled += pn
        if a.startswith("买入"): trade_count += 1
        if "卖出" in a:
            total_sold += 1
            if pn > 0: win_count += 1
            settled_pnl += pn
    ...
    # daily_pnl_amount / daily_pnl_pct 在实时市值刷新后再算（见下方）
    ...
    acct["daily_pnl_pct"] = round((fp + daily_settled) / max(equity_now, 1) * 100, 2)
    acct["position_exposure"] = data.get("position_exposure")

    # ── 最大回撤：正确考虑 capital_injections 的时序 ──
    _all_inj = list(data.get("capital_injections") or [])
    _total_injected = sum(float(x.get("amount", 0) or 0) for x in _all_inj)
    _true_initial = max(float(acct.get("initial_capital") or 0) - _total_injected, 100_000)
    _events: list[tuple[str, float]] = []
    for _inj in _all_inj:
        _events.append((str(_inj.get("at", "")), float(_inj.get("amount", 0) or 0)))
    for _t in trades:
        if "卖出" in (_t.get("action") or ""):
            _pnl = float(_t.get("pnl", 0) or 0)
            _events.append((_t.get("time", ""), _pnl))
    _events.sort(key=lambda x: x[0])
    eq_curve = [_true_initial]
    _ce = _true_initial
    for _, _delta in _events:
        _ce += _delta
        eq_curve.append(_ce)
    eq_curve.append(_ce + fp)
    _peak = eq_curve[0]
    _dd = 0.0
    for _eq in eq_curve:
        if _eq > _peak:
            _peak = _eq
        if _peak > 0:
            _d = (_peak - _eq) / _peak * 100
            if _d > _dd:
                _dd = _d
    acct["max_drawdown"] = round(-_dd, 2)
...
async def update_paper_trading(
    ...
    if "trade_log_entry" in payload:
        entry = payload["trade_log_entry"]
        action = str(entry.get("action", "") or "")
        # A股 T+1 保护：禁止当天买入的同支股票当日卖出
        if "卖出" in action:
            _today_str = datetime.now().strftime("%Y-%m-%d")
            _sym = str(entry.get("symbol", "") or "")
            for _s in current.get("strategies", []):
                for _p in _s.get("positions", []):
                    if _p.get("symbol") == _sym:
                        _bd = str(_p.get("buy_date") or "")[:10]
                        if _bd == _today_str:
                            raise HTTPException(
                                status_code=400,
                                detail=f"A股 T+1 禁止：{_sym} 今日 {_bd} 买入，不可同日卖出",
                            )
        current.setdefault("trade_log", [])
        current["trade_log"].append(entry)
