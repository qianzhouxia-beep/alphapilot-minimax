...
import { fetchLiveOrders } from "@/lib/cn-api";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) setPendingN(lo.pending_n || 0);
      } catch {
        if (!cancelled) setPendingN(0);
      }
    };
    load();
  ...
  return (
    <header className="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
      <Link href="/" className="flex items-center gap-3">
        <Image src="/brand-logo.png" alt="AlphaPilot" className="h-9 sm:h-10 w-auto" width={180} height={40} priority />
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-[#1D2A42] bg-[#0C1728] p-0.5">
          ...
          >
            ...
            量化模拟盘
            {pendingN > 0 ? (
              <span className="text-[9px] px-1.5 py-0.5 rounded-sm bg-[rgba(245,196,81,0.2)] text-[#F5C451] font-semibold border border-[rgba(245,196,81,0.35)]">
                待确认 {pendingN}
              </span>
            ) : (
