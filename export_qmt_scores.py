...
def export_fullpool_live():
    ...
    # 2026-09-09 强化：live_fund_flow 源头已把越界 f84/f170 置 None（主动买占比/涨跌幅
    # 不再喂垃圾）；此处保留强制 fail 作最后防线，并把每次命中的原始 live/base 值落盘
    # {date}.data_alert.json，供次日盘后核对（09-02 全 31 只 / 09-09 12/19 静默发作，
    # 曾整条清空 Track B 买入侧——有告警文件才不会再次"静默"）。
    def _num_ok(x, lo, hi):
        return isinstance(x, (int, float)) and not (x != x) and lo <= float(x) <= hi

    n_data_err = 0
    data_alerts = []
    for r in rows:
        abr_l = r.get("live_abr")
        abr_b = r.get("active_buy_ratio")
        chg_l = r.get("live_change_pct")
        chg_b = r.get("change_pct")
        # 取值已含 live→base 合并（上游），此处统一用最终值判；NaN 一律视为异常
        abr = abr_b if _num_ok(abr_b, 0.0, 1.0) else abr_l
        chg = chg_b if _num_ok(chg_b, -30.0, 30.0) else chg_l
        errs = []
        if not _num_ok(abr, 0.0, 1.0):
            errs.append(f"abr={abr!r}异常")
        if not _num_ok(chg, -30.0, 30.0):
            errs.append(f"chg={chg!r}异常")
        if errs:
            n_data_err += 1
            ...
            data_alerts.append({
                ...
                "rank": r.get("rank"), "errs": errs,
                "abr": abr, "chg": chg,
            })
    if n_data_err:
        print(f"[FULLPOOL_LIVE] 数据异常强制 fail: {n_data_err} 只", flush=True)
        try:
            _alert_path = OUT_DIR / (datetime.now().strftime("%Y%m%d") + ".data_alert.json")
            _alert_path.write_text(
                json.dumps({"date": datetime.now().strftime("%Y%m%d"),
                            "asof": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "n": len(data_alerts), "rows": data_alerts},
                           ensure_ascii=False, indent=2),
                encoding="utf-8")
            print(f"[FULLPOOL_LIVE] 数据告警已落盘 {_alert_path}", flush=True)
        except Exception as _e:
            print(f"[FULLPOOL_LIVE] 数据告警落盘失败: {_e}", flush=True)
