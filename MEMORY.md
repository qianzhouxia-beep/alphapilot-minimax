...
- 交易端版本：Track A 实盘 `TrackA_track_a_qmt_full_chain_live_v2.38-tpl.py`（v2.38-tpl）/ 模拟 `TrackA_track_a_qmt_full_chain_sim_v2.45.py`（v2.45）；Track B `TrackB_track_b_qmt_auction_live_v2.7-tpl.py` / `TrackB_track_b_qmt_auction_sim_v2.14.py`（**老板 QMT 部署此份**）。实盘落后模拟若干版本属正常。
