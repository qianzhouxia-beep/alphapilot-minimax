...
- 交易端版本：Track A 实盘 `TrackA_track_a_qmt_full_chain_live_v2.38-tpl.py`（v2.38-tpl）/ 模拟 `TrackA_track_a_qmt_full_chain_sim_v2.46.py`（v2.46，含 Fix C 成交确认）；Track B `TrackB_track_b_qmt_auction_live_v2.7-tpl.py` / `TrackB_track_b_qmt_auction_sim_v2.14.py`（v2.14，Fix C；**老板 QMT 部署此份**）。实盘落后模拟若干版本属正常。
