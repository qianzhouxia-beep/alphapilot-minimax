// AlphaPilot A 股 Dashboard — V18 Fusion 决策系统 (2026-07-13)
...
export default function CNDashboard() {
  ...
  // 最佳机会：从拉升阶段的股票中选评分最高者（已有主力启动，趋势确认）
    ...
    <main className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 min-h-screen">
      ...
      {data && (
        <section className="mb-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          ...
          <KPI label="全量扫描" value={`${data.stats.total_scanned || data.stats.universe_n || "—"}`} sub={`启动形态 ${data.stats.launch_hits ?? data.stats.valid_scored ?? "—"} 只 · 有效评分 ${data.stats.valid_scored ?? "—"} · ${(Number(data.stats.elapsed_seconds || 0) / 60).toFixed(0)}m · V3.1`} accent="#F5C451" />
          <KPI label="收藏追踪" value={`${wlData.length}`} sub={`${wlData.filter(w=>w.status==='active').length} 只追踪中 · 自动回测`} accent="#F5C451" />
        </section>
      )}
      ...
      <section className="glass rounded-2xl p-4 sm:p-6 mb-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-[20px] font-semibold">A 股 Top 5 机会</h2>
            <p className="mt-0.5 text-[12px] text-[#6E7C93]">
              V18 Fusion 决策系统 · 30维融合特征 · 5模型集成 · 全A股扫描 · 凌晨5:00选股 · 资金门控
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/cn/watchlist" className="rounded-lg border border-[#1D2A42] bg-[#0C1728] px-3 py-1.5 text-[12px] text-[#F5C451] hover:border-[#F5C451] transition-colors">
              ⭐ 收藏追踪
            </Link>
            <button
      ...
      {catData && (
        <section className="mb-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-[20px] font-semibold">📊 资金阶段分类</h2>
              <p className="mt-0.5 text-[12px] text-[#6E7C93]">
      ...
      {wlData.length > 0 && (
        <section className="glass rounded-2xl p-4 sm:p-6 mb-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-[20px] font-semibold">⭐ 收藏追踪</h2>
              <p className="mt-0.5 text-[12px] text-[#6E7C93]">
      ...
      {/*** 尾盘选股 ***/}
      <section className="glass rounded-2xl p-4 sm:p-6 mb-6 border-l-[3px] border-l-[#8B5CF6]">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="18" cy="18" r="16" stroke="#7C3AED" strokeWidth="1.5" fill="rgba(139,92,246,0.08)" />
              <line x1="18" y1="4" x2="18" y2="32" stroke="#7C3AED" strokeWidth="0.8" opacity="0.35" />
              <line x1="4" y1="18" x2="32" y2="18" stroke="#7C3AED" strokeWidth="0.8" opacity="0.35" />
              <circle cx="18" cy="18" r="2.5" fill="#A78BFA" opacity="0.9" />
              <line x1="10.5" y1="24.5" x2="10.5" y2="13" stroke="#3EE6A8" strokeWidth="2" strokeLinecap="round" />
              <rect x="9.5" y="20" width="2" height="4.5" rx="0.5" fill="#3EE6A8" />
              <line x1="14.5" y1="24.5" x2="14.5" y2="10" stroke="#FF5D5D" strokeWidth="2" strokeLinecap="round" />
              <rect x="13.5" y="16" width="2" height="3" rx="0.5" fill="#FF5D5D" />
              <line x1="18.5" y1="24.5" x2="18.5" y2="9" stroke="#3EE6A8" strokeWidth="2" strokeLinecap="round" />
              <rect x="17.5" y="14" width="2" height="4" rx="0.5" fill="#3EE6A8" />
              <line x1="22.5" y1="24.5" x2="22.5" y2="15" stroke="#FF5D5D" strokeWidth="2" strokeLinecap="round" />
              <rect x="21.5" y="19" width="2" height="3" rx="0.5" fill="#FF5D5D" />
              <polyline points="10.5,20 14.5,16 18.5,12 22.5,18" stroke="#A78BFA" strokeWidth="1.2" fill="none" opacity="0.5" />
            </svg>
            <div>
              <h2 className="text-[18px] font-semibold">尾盘狙击 · 14:55</h2>
            <p className="mt-0.5 text-[12px] text-[#6E7C93]">
              S2最优策略 · 涨幅+量比+均线多头+站上VWAP · 按波动率排序选Top3
            </p>
          </div>
          <span className="text-[10px] px-2 py-1 rounded-full bg-[rgba(139,92,246,0.12)] text-[#A78BFA] border border-[rgba(139,92,246,0.3)]">
            尾盘选股
          </span>
        </div>
        <p className="text-[13px] text-[#6E7C93] italic">⏰ 每日 14:30 自动筛选，14:55 输出推荐</p>
      </section>

      {/*** 自我学习功能亮点 ***/}
      <section className="mb-6">
        <h2 className="text-[18px] font-semibold mb-3">🧠 自我进化学习</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="glass rounded-2xl p-4 card-lift border-t-2 border-t-[#3EE6A8]">
            <div className="text-[24px] mb-1">📊</div>
            <h3 className="text-[14px] font-semibold text-[#EAF2FF] mb-1">胜率自动统计</h3>
            <p className="text-[11px] text-[#6E7C93] leading-relaxed">
          ...
          <div className="glass rounded-2xl p-4 card-lift border-t-2 border-t-[#F5C451]">
            <div className="text-[24px] mb-1">⚡</div>
            <h3 className="text-[14px] font-semibold text-[#EAF2FF] mb-1">门控参数优化</h3>
            <p className="text-[11px] text-[#6E7C93] leading-relaxed">
      ...
      {priceDialog && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm"
          ...
          <div className="w-[90vw] max-w-[380px] rounded-2xl border border-[#1D2A42] bg-[#0C1728] p-6 shadow-2xl mx-4"
            ...
            <h3 className="text-[18px] font-semibold text-[#EAF2FF] mb-1">
              ⭐ 添加收藏
            </h3>
...
// ═══ 资金阶段 SVG 图标 — Geometric Precision 风格 ═══
function PhaseIcon({ phaseKey, color, size = 24 }: { phaseKey: string; color: string; size?: number }) {
  ...
  switch (phaseKey) {
    case "markup": // 拉升 — 向上的箭头
      return <svg {...s}><path d="M12 21V5"/><polyline points="5 12 12 5 19 12"/><line x1="12" y1="5" x2="12" y2="7"/></svg>;
    case "rightside_ambush": // 右侧潜伏 — 靶心
      return <svg {...s}><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><circle cx="12" cy="12" r="1.5" fill={color}/><line x1="12" y1="1" x2="12" y2="4"/><line x1="12" y1="20" x2="12" y2="23"/><line x1="1" y1="12" x2="4" y2="12"/><line x1="20" y1="12" x2="23" y2="12"/></svg>;
    case "accumulation_end": // 吸筹末期 — 铃铛
      return <svg {...s}><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>;
    case "bear_trap": // 诱空陷阱 — V型陷阱+诱饵
      return <svg {...s}><polyline points="4 20 12 6 20 20"/><circle cx="12" cy="4" r="2.5" fill={color} stroke="none"/><line x1="12" y1="20" x2="12" y2="12"/></svg>;
    case "accumulation": // 吸筹 — 向下汇聚
      return <svg {...s}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 9 12 14 17 9"/><line x1="12" y1="14" x2="12" y2="2"/></svg>;
    case "suspicious": // 诱多嫌疑 — 三角问号
      return <svg {...s}><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><circle cx="12" cy="16" r="0.5" fill={color}/><line x1="12" y1="9" x2="12" y2="13"/></svg>;
    case "distribution": // 出货 — 向上发散
      return <svg {...s}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="14"/></svg>;
    case "pullback": // 回调 — 向下折返
      return <svg {...s}><path d="M21 17c-4 5-11 5-15 2S2 10 3 6"/><polyline points="7 13 3 10 7 7"/></svg>;
    case "sideways": // 震荡 — 水平波动
      return <svg {...s}><polyline points="3 10 7 14 12 10 17 14 21 10"/><polyline points="3 14 7 10 12 14 17 10 21 14"/></svg>;
    default:
      return <svg {...s}><circle cx="12" cy="12" r="8"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>;
  }
}

// ═══ 资金阶段分类卡片 ═══
const PHASE_COLORS: Record<string, string> = {
  bear_trap: "#8B5CF6",      // 紫色 - 诱空
  rightside_ambush: "#F59E0B", // 金色 - 右侧潜伏
  accumulation_end: "#10B981",  // 绿色 - 吸筹末期
  markup: "#EF4444",           // 红色 - 拉升
  accumulation: "#3B82F6",     // 蓝色 - 吸筹
  suspicious: "#F97316",       // 橙色 - 诱多嫌疑
  distribution: "#DC2626",     // 深红 - 出货
  pullback: "#6B7280",         // 灰色 - 回调
  sideways: "#9CA3AF",         // 浅灰 - 震荡
};
...
const PHASE_GROUPS = [
  {
    key: "buy_signal",
    label: "买入信号",
    desc: "主力资金正在运作，关注买入机会",
    color: "#EF4444",
    phases: ["markup", "rightside_ambush", "accumulation_end", "bear_trap"]
  },
  {
    key: "accumulation_watch",
    label: "吸筹观察",
    desc: "主力在低位默默吸筹",
    color: "#3B82F6",
    phases: ["accumulation"]
  },
  {
    ...
    desc: "警惕回调或出货风险",
    color: "#F97316",
    phases: ["suspicious", "distribution"]
  },
  {
    key: "wait_and_see",
    label: "暂时观望",
              ...
              {stocks.length === 0 ? (
                ...
                <div className="space-y-1">
                  {stocks.slice(0, 5).map((s: any, i: number) => {
                    ...
                    const sScore = s.score || s.score_pct / 100 || 0;
                    const sColor = sScore >= 0.50 ? "text-[#3EE6A8]" : sScore >= 0.40 ? "text-[#4DA3FF]" : sScore >= 0.30 ? "text-[#F5C451]" : "text-[#9FB0C7]";
                    return (
                      <div key={s.symbol} className="grid grid-cols-[20px_1fr_42px_55px] sm:grid-cols-[22px_1fr_42px_1fr_85px_70px_26px] items-center rounded-lg bg-[#121c2a] p-1.5 hover:bg-[#16202f] transition-colors group gap-1">
                        ...
                        {/* 信心分 */}
                        <span className={`font-display-numeric text-[11px] font-bold text-center ${sColor}`}>
                          {Math.min(99, Math.max(75, Math.round(sScore * 45 + 75)))}
                        </span>
